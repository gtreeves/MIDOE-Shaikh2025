...
%old results before removing GFP terms
% filename = "trimer_model_og_isres_fits.mat";


%old results with GFP terms and entire Smad4 profile from Warmflash
% filename = "2025-01-13-db_smad4.mat";

%only calibrated with Smad2
% filename = '2025-01-31-db_v0.mat';


%entire Smad4 and product of SSE
% filename = '2025-01-14-db_smad4_noGFP_alt_objFcn.mat';

%product of SSE
% filename = '2025-01-14-db_smad4_noGFP_alt_objFcn_smad4_2';

%same as previous run for 3000G
% filename = '2025-01-14-db_smad4_noGFP_alt_objFcn_smad4_2_3000G';

% same as previous (product of SSE) normalized by range
% filename = '2025-01-25-db_smad4_nornRange.mat';

%same as previous add obj function
% filename = '2025-01-26-db_smad4_nornRange_add.mat';

%change bounds to previous
% filename = '2025-01-27-db_smad4_nornRange_add_bounds.mat';

% SSE(Smad2)+SSE(Smad4)
% filename = '2025-02-07-db_v5_addSSEs_noNorm';
% 1.5*SSE(Smad2)+SSE(Smad4)
% filename = '2025-01-29-db_normRange_add_weigh15smad2.mat';

% 2*SSE(Smad2)+SSE(Smad4)
% filename = '2025-01-31-db_normRange_add_weigh02smad2.mat';

%bounds based on PLL from Smad2-Smad4
% filename = '2025-02-01-db_normRange_v5boundsPLL.mat';
% }
%new smad4 i.e. v3 (manually adjusting points)
% filename = '2025-02-08-db_smad4v3_addObjs.mat';

%new smad4 i.e. v3 without smoothing
% filename = '2025-02-08-db_smad4v4_addObjs.mat';

%carefully calc delay to make smad4_v5
% filename = '2025-02-08-db_smad4v5_addObjs.mat';

%carefully add and subtract delay
% filename = '2025-02-14-db_smad4v6_addObjs.mat';

%smad7 -- use smad2 and smad4 from WF data
% filename = '2025-04-17-db_v7_2.mat';
% filename = '2025-04-22-db_7_2';
%7_2_FRAP using schmierer2005 FRAP with WF Smad2 dynamics
filename = '2025-04-20-db_FRAP_7_2';
...
for i=1:length(BM)
    ...
    sol = ode15s(@Smad_model_trimer_screen_smad4,0:0.1:150,y0,[],ka);
    %         if max(sol.x) < 45
    %             error('ode15s failed')
    %         end
    ...
    % y0(2) = 0.066; y0(5) = 1000;
    sol_SB = ode15s(@Smad_model_trimer_screen_smad4_SB,60.1:0.1:600,Yn(end,:),[],ka);
    %         if max(sol_SB.x) < 150
    %             error('ode15s failed')
    %         end
...
% hold on 
errorbar(t_smad4,smad4_avg,smad4_std,smad4_std,'.','Color','g')
% xticks(0:50:300)
axis('square')
xlabel('time [min]')
...
figure(Ftrimer)
hold on
plot(0:0.1:150,trimer(:,i),'Color','k')
% xticks(0:10:50)
axis('square')
...
% nexttile(4)
% hold on
% plot(0:0.1:300,coLoc(:,i),"Color",'k');hold on; 
% axis('square')
end
% title(t,filename)
set(gca,'FontSize',8,'FontName','Arial')
