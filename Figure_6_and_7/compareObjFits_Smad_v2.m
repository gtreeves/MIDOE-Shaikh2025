...
load("v8.mat") %colocalization data
tspan3 = stf;
stcoLocdata_avg = stcoLoc_avg;
stcoLocdata_std = stcoLoc_std;
...
%{
    ...
    %% coLoc expt
    solcoLoc = ode15s(@Model_before_SB,0:0.1:45,y0,[],ka);
    %         if max(sol.x) < 45
    %             error('ode15s failed')
    %         end
...
errorbar(stf,stcoLocdata_avg,stcoLocdata_std,stcoLocdata_std,'Marker','.','LineStyle','none','Color','g')
axis('square')
xlabel('time [min]')
ylabel('coLoc [nM]')
set(gca,'FontSize',8,'FontName','Arial')
...
exportgraphics(Fcoloc,[filename,'_smad4','.eps'],'ContentType','vector')
