...
function [F,Phi] = objective_Smad_v3(k)
...
parfor i = 1:nsets
    ...
    % error_Smad_complex = sum(((Smad24n_calc - Smad24n)./sd).^2,'all');
        %}
        %% trimer expt
        %     strimer = ode15s(@Smad_model_trimer_screen_smad4,0:0.1:45,y0,[],ka);
        %         if max(sol.x) < 45
        %             error('ode15s failed')
        %         end
        Yntrimer = (deval(sol,tspan3))';
        trimer = Yntrimer(:,17);

        error_all_trimer = sum(((trimer - strimerdata_avg)./strimerdata_std).^2,'all');

        %% Calc error
        F(i) =  error_all_Smad2n + error_all_trimer; % + error_Smad_complex;%+ sum((ycalc_e(:,:,i) - yvalse).^2) + sum((ycalc_f(:,:,i) - yvalsf).^2);
    catch ME
        F(i) = 1e7;
