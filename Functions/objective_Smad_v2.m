...
function [F,Phi] = objective_Smad_v2(k)
...
parfor i = 1:nsets
    ...
    % error_Smad_complex = sum(((Smad24n_calc - Smad24n)./sd).^2,'all');
        ...
        %             error('ode15s failed')
        %         end
        YncoLoc = (deval(sol,tspan3))';
        coLoc = (YncoLoc(:,15) +YncoLoc(:,17));

        error_all_coLoc = sum(((coLoc - stcoLocdata_avg)./stcoLocdata_std).^2,'all');

        %% Calc error
        F(i) =  error_all_Smad2n + error_all_coLoc;% + error_Smad_complex;%+ sum((ycalc_e(:,:,i) - yvalse).^2) + sum((ycalc_f(:,:,i) - yvalsf).^2);
    catch
        F(i) = 1e5;
    end
end
