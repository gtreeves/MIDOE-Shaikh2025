...
function [F,Phi] = objective_Smad_v0(k)
...
parfor i = 1:nsets
    ...
    try
        ...
        sol = ode15s(@Model_before_SB,0:0.1:60,y0,[],ka);
        %         if max(sol.x) < 45
        %             error('ode15s failed')
        %         end
        Yn = (deval(sol,tspan1))';
        ...
        % y0(2) = 0.066; y0(5) = 1000;
        sol_SB = ode15s(@Model_after_SB,60:0.1:600,Yn(end,:),[],ka);
        %         if max(sol_SB.x) < 150
        %             error('ode15s failed')
        %         end
    ...
    % error_Smad_complex = sum(((Smad24n_calc - Smad24n)./sd).^2,'all');
        %}
        %% Calc error
        F(i) =  error_all_Smad2n;% + error_Smad_complex;%+ sum((ycalc_e(:,:,i) - yvalse).^2) + sum((ycalc_f(:,:,i) - yvalsf).^2);
    catch ME
        F(i) = 1e7;
    end
end
